...
public class ItemControllerTest extends BaseIntegrationTest{
	...
	class GetTest{
		...
		@Test 
		void whenMakeSearch(){
			ItemDataTransferObject item5 = itemMapper.toDto(itemRepository.save(
				Item.builder().name("test").description("description").primaryTag(ItemTag.POPULAR).build()
			));	
			...
			ItemSearchCriteria search = ItemSearchCriteria.builder().description("desc")
				.primaryTag("POPULAR").size(10).page(0)
				.tags(List.of(ItemTag.POPULAR.toString()))
				.build();

			ItemSearchCriteria searchByTags = ItemSearchCriteria.builder()
				.tags(List.of(ItemTag.POPULAR.toString()))
				.build();

			ItemSearchCriteria searchByPrimaryTag = ItemSearchCriteria.builder()
				.primaryTag(ItemTag.POPULAR.toString())
				.build();


			given().contentType(ContentType.JSON).body(search)
				.when().get("/items/search").then().statusCode(200)
				.and()
				.body("content", notNullValue())
				.body("content.size()", greaterThanOrEqualTo(0))
				.body("content[0].name", equalTo(item5.getName()));
	
			/*given().contentType(ContentType.JSON).body(searchByTags)
				.when().get("/items/search").then().statusCode(200)
				.and()
				.body("content", notNullValue());
			*/
			
			/*given().contentType(ContentType.JSON).body(searchByPrimaryTag)
				.when().get("/items/search").then().statusCode(200)
				.and()
				.body("content", notNullValue());
			*/

			ItemSearchCriteria searchBad = ItemSearchCriteria.builder().build();

			given().contentType(ContentType.JSON).body(searchBad)
				.when().get("/items/search").then().statusCode(400);
