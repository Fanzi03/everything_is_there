...
public interface ItemRepository extends JpaRepository<Item, UUID> {
	...
	@Query("""
		...
		WHERE 
		(CASE 
			WHEN :description IS NULL OR :description= '' THEN TRUE
			ELSE LOWER(i.description) LIKE LOWER(CONCAT('%', LOWER(COALESCE(:description, '')), '%'))
		END) 
		AND 
		(:primaryTag IS NULL OR i.primaryTag = :primaryTag) 
		AND (:tags IS NULL OR t IN :tags)
		""")
	Page<Item> search(@Param("description") String description, 
		@Param("primaryTag") ItemTag primaryTag, @Param("tags") Set<ItemTag> tags, Pageable pageable
	);
}
